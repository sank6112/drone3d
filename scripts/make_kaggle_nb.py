...
cells = [
 ...
 code("import os, subprocess, glob, json\n",
      ...
      "else:  # persistence ON -> force-sync to latest master (discard any in-place edits)\n",
      "    subprocess.run(['git','-C','drone3d','fetch','origin','master'])\n",
      "    subprocess.run(['git','-C','drone3d','reset','--hard','FETCH_HEAD'])\n",
